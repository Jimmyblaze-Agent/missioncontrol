  ...
  try {
    ...
    switch (action) {
      ...
      case 'heartbeat': {
        ...
        // Ping the main site
        try {
          const { stdout: ping } = await execAsync('curl -s -o /dev/null -w "%{http_code}" --max-time 5 https://greenox.digital');
          results.push(`\n🌐 greenox.digital: HTTP ${ping.trim()}`);
        } catch {
          results.push('\n🌐 greenox.digital: unreachable');
        }
